    ...
    <div className="flex gap-3">
      ...
      <div className="flex-1 min-w-0 py-0.5">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {item.count > 0 && (
            <span className="text-xs text-gray-400">{item.count} product{item.count !== 1 ? 's' : ''}</span>
          )}
        </div>
        <p className="text-xs text-gray-500 mt-0.5">{item.description}</p>
        {item.action && (
          <p className="text-xs text-gray-700 mt-1 font-medium">→ {item.action}</p>
        )}
      </div>
    </div>
  );
    ...
    <Layout>
      <div className="p-4 lg:p-6 space-y-5">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {[...Array(4)].map((_, i) => <div key={i} className="h-48 bg-white rounded-2xl border border-gray-100 animate-pulse" />)}
        </div>
      </div>
    </Layout>
  );
      ...
      <div className="p-4 lg:p-6 space-y-5">
        ...
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">Actionable recommendations based on your competitive data</p>
          </div>
          <button
            onClick={fetchInsights}
            className="inline-flex items-center gap-2 px-4 py-2.5 bg-white border border-gray-200 hover:border-gray-300 text-gray-700 rounded-xl text-sm font-medium transition-colors shadow-sm"
          >
            {Ico.refresh} Refresh
          </button>
        </div>
        ...
        {insights?.priorities?.length > 0 && (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-50">
              ...
              <p className="text-xs text-gray-500 mt-0.5">Actions you should take right now</p>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          {insights?.opportunities?.length > 0 && (
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              <div className="px-5 py-4 border-b border-gray-50 flex items-center gap-2">
                <div className="w-7 h-7 bg-emerald-50 rounded-lg flex items-center justify-center text-emerald-600">{Ico.up}</div>
                <div>
                  <h2 className="text-sm font-semibold text-gray-900">Opportunities</h2>
                  <p className="text-xs text-gray-500">Revenue growth potential</p>
                </div>
              </div>
              <div className="p-5 space-y-4">
          ...
          {insights?.threats?.length > 0 && (
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              <div className="px-5 py-4 border-b border-gray-50 flex items-center gap-2">
                <div className="w-7 h-7 bg-red-50 rounded-lg flex items-center justify-center text-red-500">{Ico.warn}</div>
                <div>
                  <h2 className="text-sm font-semibold text-gray-900">Threats</h2>
                  <p className="text-xs text-gray-500">Competitive risks to watch</p>
                </div>
              </div>
              <div className="p-5 space-y-4">
                {insights.threats.map((threat, i) => (
                  <div key={i} className="flex gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <p className="text-sm font-semibold text-gray-900">{threat.title}</p>
                        {threat.severity && (
                      ...
                      <p className="text-xs text-gray-500 mt-0.5">{threat.description}</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {insights?.trending?.length > 0 && (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-50 flex items-center gap-2">
              <div className="w-7 h-7 bg-amber-50 rounded-lg flex items-center justify-center text-amber-600">{Ico.fire}</div>
              <div>
                <h2 className="text-sm font-semibold text-gray-900">Trending Products</h2>
                <p className="text-xs text-gray-500">Products with high market activity</p>
              </div>
            </div>
              ...
              {insights.trending.map((product, i) => (
                <div key={i} className="px-5 py-3 flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-gray-900 truncate">{product.product_title}</p>
                    <p className="text-xs text-gray-500 truncate">{product.reason}</p>
                  </div>
                  <span className="shrink-0 px-2.5 py-0.5 bg-amber-50 text-amber-700 rounded-full text-xs font-medium">
        ...
        {!loading && !error && !insights?.priorities?.length && !insights?.opportunities?.length && !insights?.threats?.length && (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-16 text-center">
            ...
            <p className="text-xs text-gray-400 mt-1">Add products and competitors to start getting actionable recommendations</p>
          </div>
        )}
