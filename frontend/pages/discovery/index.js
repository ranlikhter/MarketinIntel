      ...
      <div className="p-5">
        ...
        <div className="flex items-start justify-between gap-2 mb-3">
          <div className="min-w-0">
            <h3 className="text-sm font-semibold text-gray-900 truncate">{suggestion.product_title || suggestion.title}</h3>
            {suggestion.competitor_name && (
              <p className="text-xs text-gray-500 mt-0.5">{suggestion.competitor_name}</p>
            )}
          </div>
        ...
        {suggestion.price != null && (
          <p className="text-sm font-bold text-gray-900 mb-1">${suggestion.price.toFixed(2)}</p>
        )}

        {/* Reason */}
        {suggestion.match_reason && (
          <p className="text-xs text-gray-400 line-clamp-2 mb-4">{suggestion.match_reason}</p>
        )}
        ...
        <div className="flex items-center gap-2 pt-3 border-t border-gray-50">
          ...
          <button
            ...
            disabled={actioning}
            className="flex-1 py-2 border border-gray-200 hover:border-gray-300 text-gray-600 hover:text-gray-900 rounded-xl text-xs font-semibold transition-colors disabled:opacity-50"
          >
            Reject
          </button>
        </div>
  ...
  if (loading) return (
    <Layout>
      <div className="p-4 lg:p-6 space-y-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">{[...Array(6)].map((_, i) => <div key={i} className="h-44 bg-white rounded-2xl border border-gray-100 animate-pulse" />)}</div>
      </div>
    </Layout>
  );
  ...
  return (
    <Layout>
      <div className="p-4 lg:p-6 space-y-5">
        ...
        {/* Header */}
        <div>
          <h1 className="text-xl font-bold text-gray-900">Auto Discovery</h1>
          <p className="text-sm text-gray-500 mt-0.5">Find competitor listings automatically with confidence-scored matching</p>
        </div>

        {/* Stats */}
        <div className="grid grid-cols-3 gap-4">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex items-center gap-4">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0 bg-blue-50 text-blue-600">{Ico.search}</div>
            <div><p className="text-2xl font-bold text-gray-900 leading-none">{suggestions.length}</p><p className="text-xs text-gray-500 mt-1">Total Suggestions</p></div>
          </div>
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex items-center gap-4">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0 bg-amber-50 text-amber-600">{Ico.pending}</div>
            <div><p className="text-2xl font-bold text-gray-900 leading-none">{pending.length}</p><p className="text-xs text-gray-500 mt-1">Pending Review</p></div>
          </div>
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex items-center gap-4">
            ...
            <div><p className="text-2xl font-bold text-gray-900 leading-none">{approved.length}</p><p className="text-xs text-gray-500 mt-1">Approved</p></div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-50">
            <h2 className="text-sm font-semibold text-gray-900">Discover Competitors</h2>
            <p className="text-xs text-gray-500 mt-0.5">Search by product or keyword to find matching competitor listings</p>
          </div>
          <div className="p-5 flex flex-wrap gap-3">
            ...
            <div className="flex-1 min-w-48">
              <label className="block text-xs font-medium text-gray-600 mb-1.5">Keyword (optional)</label>
              <input
                value={keyword}
            ...
            <div className="flex items-end">
              <button
                ...
                disabled={searching}
                className="inline-flex items-center gap-2 px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-sm font-medium transition-colors disabled:opacity-50"
              >
        ...
        {pending.length > 0 && (
          <div>
            <h2 className="text-sm font-semibold text-gray-700 mb-3">
              Pending Review <span className="text-gray-400 font-normal">({pending.length})</span>
            </h2>
        ...
        {approved.length > 0 && (
          <div>
            <h2 className="text-sm font-semibold text-gray-700 mb-3">
              Approved <span className="text-gray-400 font-normal">({approved.length})</span>
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
              {approved.map(s => (
                <div key={s.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex items-start gap-3 opacity-75">
                  <div className="w-8 h-8 bg-emerald-50 rounded-xl flex items-center justify-center text-emerald-600 shrink-0">{Ico.check}</div>
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-gray-900 truncate">{s.product_title || s.title}</p>
                    {s.competitor_name && <p className="text-xs text-gray-500">{s.competitor_name}</p>}
        ...
        {suggestions.length === 0 && (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-16 text-center">
            <div className="w-14 h-14 bg-gray-100 rounded-2xl flex items-center justify-center mx-auto mb-4 text-gray-300">{Ico.search}</div>
            <p className="text-sm font-medium text-gray-900">No suggestions yet</p>
            <p className="text-xs text-gray-400 mt-1">Use the search panel above to discover competitor listings</p>
          </div>
        )}
