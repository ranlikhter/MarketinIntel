...
function ProductCard({ product, matches }) {
  ...
  return (
    <div
      className={`bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden ring-1 ${pos.ring} hover:shadow-md transition-shadow`}
    >
      {/* Colour bar */}
      ...
      <div className="p-4">
        ...
        <div className="bg-gray-50 rounded-xl p-3 mb-3 space-y-1.5">
          <div className="flex items-center justify-between text-xs">
            <span className="text-gray-500">My price</span>
            <span className="font-semibold text-gray-900">
              {myPrice != null ? `$${myPrice.toFixed(2)}` : '—'}
            </span>
          </div>
          <div className="flex items-center justify-between text-xs">
            <span className="text-gray-500">Lowest competitor</span>
            <span className="font-semibold text-gray-900">
        ...
        <div className="flex gap-2">
          <Link
            href={`/products/${product.id}`}
            className="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold rounded-lg transition-colors"
          >
            View Details {Ico.arrow}
          </Link>
          <button
            onClick={() => downloadCSV(product)}
            className="inline-flex items-center justify-center w-9 h-9 rounded-lg border border-gray-200 hover:bg-gray-50 text-gray-500 hover:text-gray-700 transition-colors"
            title="Download CSV"
          >
            {Ico.download}
          </button>
...
function SummaryCard({ label, count, total, color, dotColor, onClick, active }) {
  ...
  return (
    <button
      onClick={onClick}
      className={`text-left bg-white rounded-2xl border shadow-sm p-5 flex items-center gap-4 w-full transition-all hover:shadow-md ${
        active ? 'border-blue-300 ring-2 ring-blue-100' : 'border-gray-100'
      }`}
    >
      ...
      <div className="text-right">
        <p className="text-sm font-semibold text-gray-400">{pct}%</p>
      </div>
    </button>
  );
}
      ...
      <div className="p-4 lg:p-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-5">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2.5 rounded-xl border border-gray-200 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 transition-shadow bg-white"
            />
          </div>
          <div className="relative">
            ...
            <select
              ...
              className="pl-9 pr-8 py-2.5 rounded-xl border border-gray-200 text-sm text-gray-900 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 transition-shadow appearance-none"
            >
        ...
        {!loading && (
          <div className="mt-8 bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            <h2 className="text-sm font-semibold text-gray-700 mb-3">Position Legend</h2>
