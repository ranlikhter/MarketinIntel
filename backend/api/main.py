...
API_VERSION = "1.1.0"
SAFE_DEBUG_ENVS = {"development", "dev", "local", "test", "testing"}


def _normalize_app_env(app_env: str | None = None) -> str:
    """Normalize the deployment environment for security decisions."""
    return (app_env or os.getenv("APP_ENV") or "development").strip().lower()


def _debug_surfaces_enabled(app_env: str | None = None) -> bool:
    """Expose Swagger/OpenAPI and detailed health data only in safe local/test envs."""
    return _normalize_app_env(app_env) in SAFE_DEBUG_ENVS


def _build_deep_health_response(checks: dict, app_env: str | None = None) -> JSONResponse:
    """Hide raw dependency failures outside trusted local/test environments."""
    overall = "healthy" if all(value == "ok" for value in checks.values()) else "degraded"
    content = {"status": overall}

    if _debug_surfaces_enabled(app_env):
...
def create_app(app_env: str | None = None) -> FastAPI:
    """Create the FastAPI application with environment-aware security defaults."""
    resolved_env = _normalize_app_env(app_env)
    debug_surfaces_enabled = _debug_surfaces_enabled(resolved_env)

    app = FastAPI(
        title="MarketIntel API",
        description="E-commerce Competitive Intelligence Platform - Monitor ANY competitor website",
        version=API_VERSION,
        docs_url="/docs" if debug_surfaces_enabled else None,
        redoc_url="/redoc" if debug_surfaces_enabled else None,
        openapi_url="/openapi.json" if debug_surfaces_enabled else None,
    )
    ...
    async def deep_health_check():
        ...
        return _build_deep_health_response(checks, resolved_env)
